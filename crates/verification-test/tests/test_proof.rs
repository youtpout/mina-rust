...
fn test_deserialize_o1js_proof() {
    ...
    let public_inputs = parsed
        .public_input
        .iter()
        .map(|s| s.parse::<u128>().expect("parse public input"))
        .collect::<Vec<_>>();

    // At this point you have everything decoded:
    // - parsed.proof           (PicklesProofProofsVerified2ReprStableV2)
    // - parsed.public_input    (Vec<String> from o1js)
    // - parsed.public_output   (Vec<String> from o1js)
    // - verification_key       (VerificationKey)
    // - verifier_index         (for Kimchi)
    // - deferred_values        (Pickles deferred values)
